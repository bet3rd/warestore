...
- **🏆 Automatic matchmaking rank fetch (GCPD)** — pull each account's CS2 **Premier** and **Wingman** rank, win count, and competitive-cooldown status straight from Steam's matchmaking data. Fetch a single account, a multi-selection, or the whole roster on demand — sequentially, one at a time — or automatically on launch, with the rank and cooldown shown right on the card.
